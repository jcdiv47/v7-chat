...
| Skill source files | `agent-skills/*/SKILL.md` and `agent-skills/*/references/*.md` | Source of truth for loaded skill content. `src/lib/skills/fs-source.ts` strips frontmatter, then appends each reference file under `## Reference: ...`. Current skills: `business-domain-analysis`, `business-answer-style`, `chart-selection`, `asking-clarifications`. |
